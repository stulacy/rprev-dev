...
test_that("incidence returns same values as before", {
    ...
    expect_ref <- function(start=NULL, years=NULL) {
        ...
        expect_equal_to_reference(incidence(prevsim$entrydate, start=start, num_reg_years=years), file=fn)
        i <<- i + 1
    }
...
test_that("incidence returns integers", {
    expect_integer <- function(start=NULL, years=NULL) {
        expect_match(typeof(incidence(prevsim$entrydate, start=start, num_reg_years=years)), 'integer')
    }
...
test_that("incidence returns no NAs", {
    expect_NA <- function(start=NULL, years=NULL) {
        expect_equal(any(is.na(incidence(prevsim$entrydate, start=start, num_reg_years=years))), FALSE)
    }
