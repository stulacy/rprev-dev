...
#'                                colnames = names)
#'
#' TODO make cure_time a number of years
prevalence <- function(form, data, num_years_to_estimate,
    ...
    if (is.null(cure_time))
        cure_time <- num_years_to_estimate + 1
    ...
    results <- lapply(levels(data_r[, sex_var]), function(x) {
        ...
        .prevalence_subgroup(sub_data[, age_var], sub_data[, entry_var], start, wb_boot, num_reg_years,
                             surv_functions[[x]], cure_time, as.numeric(x), max_yearly_incidence, num_years_to_estimate,
...
#' @examples
#' n_year_estimates(num_years_to_estimate = 3, num_reg_years = 8,
