...
The default survival models are based on the `survival::survreg` function and are optimised to improve runtime. A more flexible alternative to providing parametric survival models is to pass in `flexsurv` objects from the `flexsurv` package. This is an easily extensible framework that comes with implementations of a large number of standard parametric families in addition to other models such as Royston and Parmar's Flexible Parametric Models. 

Using `flexsurv` objects requires the creation of one in advance of any prevalence estimation and simply passing this object in to the `surv_model` argument. For example, the log-logistic distribution isn't currently supported by the default survival model in `rprev`, but it can be used in the `flexsurv` implementation.

```{r}
llog <- flexsurvreg(Surv(time, status) ~ age + sex, data=prevsim, dist='llogis')
llog
```

It must be emphasized that the runtime significantly increases when using a `flexsurv` object as they have not been optimised for use in `rprev`, however, they provide greater flexibility in the survival modelling. For example, the user can model different survival models in the familiar `flexsurv` framework before using the final object in estimating prevalence.
...
The object-oriented manner in which the `prevalence` function is designed allows for custom survival and incidence objects to be passed in rather than relying on the default models. The previous section described both how to use the default models, but also how to pass in `flexsurv` survival objects. The latter works because the appropriate interface has been supplied with `rprev`, but the same mechanism can be used to provide custom objects. This section describes the requirements of the incidence and survival models.

Both of these objects have similar requirements and are used in two ways during the simulation. During each iteration, a new incidence and survival model is fit to the bootstrapped registry data, generating new parameters. Based on these parameters, each model is then used for its primary purpose, either generating an incident population or predicting the survival of this population.

The way in which the models are fit to the bootstrapped data is the same, the objects must contain a `call` object that holds the initial function call used to build the model. This call must contain an argument (name not important) which is passed the value `data`, as it is this argument which is changed to provide the bootstrapped data during simulation.
...
It is likely that a homogeneous Poisson process is sufficient for the vast majority of diseases, although if greater control is required then a custom incidence model can be provided. Incidence models are characterised by how they predict new cases of the disease will appear in the population. To this end, an incidence object needs to have an appropriate S3 class method called `draw_incident_population` that will generate the incident population, in terms of their incident times (measured by time since the origin) and any covariates that are required for the survival modelling. The method must have the following signature **NB: in this and the following section, `myobj` is used as an example class name. This can be whatever you want provided it does not clash with the existing namespace**:
...
extract_covars.myobj <- function(object) {
    ...
    # Returns a character vector detailing the covariates required to fit this model. All of these values should be columns
    # in the data that is passed in the main 'prevalence' function.
}
```
...
predict_survival_probability.myobj <- function(object, newdata, times) {
    ...
    # newdata: A data frame (or data.table) with the incident population stored with their required covariates for the model.
    # times: A vector of times to estimate survival probability at for individuals in corresponding rows of 'newdata'. 
    #  This should be the same length as there are rows in 'newdata' since each individual has their survival probability estimated once.
    
    # Returns:
    #  A vector of survival probabilities of length equal to the length of 'times' and the number of rows in 'newdata'.
}
```
