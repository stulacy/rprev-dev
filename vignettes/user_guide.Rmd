...
```{r}
cure_mod <- flexsurvcure::flexsurvcure(Surv(time, status) ~ age + sex, data=prevsim, dist='weibull', link='logistic', mixture=TRUE)
```
...
Note that a separate cure model implementation is available in the function `fixed_cure`, which specifies a *cure time* for the disease, beyond which a patient's survival is determined to have returned to population survival rates. A model is build in a very similar way to `survival::survreg`, but with the addition of specifying a cure time (in the same time-scale as survival), here we have used 5 years, which a quarter of these simulated patients survive until. However, we have no other basis for suggesting a cure model here and are only using it for demonstration of the software.

As with `flexsurvcure`, population lifetables can be provided, although by default UK mortality tables are provided with the package in `UKmortalitydays`. Please look at the documentation for `fixed_cure` or `flexsurvcure_population` if you wish to provide your own life tables.
...
Use of a fixed cure model here has increased the prevalence as patients are reverting back to population levels of survival.
...
This section provides example code for providing custom incidence and survival objects to `prevalence`.

First, a demonstration of how to build an incidence model, using a homogeneous Poisson process as an example. This process is parameterised by a single value: the rate $\lambda$, which will need to be saved in the model object along with the function call. For the purpose of this example, this model will be hardcoded to work with a particular data set (as will the vast majority of custom processes), in particular the `prevsim` simulated data set.
...
To validate that an incidence model has been correctly specified, the `test_incidence_model` function has been provided. It accepts the incidence model itself and the registry data that it has been designed to work with. It will verify that the required attributes and methods are available, and that `draw_incident_population` successfully simulates individuals.
...
The second process that can be specified by a custom model is the survival process. For this example a Weibull model with `age` as the sole covariate will be used. The survival model will be fitted using the `flexsurv` package, this functionality is already implemented as discussed earlier, but it is an appropriate demonstration.
...
Survival objects have two methods that need to be implemented. The first - `extract_covars` - is to return the names of columns in the registry data that are required by the model, so that the incidence model can generate an appropriate incident population. In this example, only `age` is required.
...
The primary functionality of a survival model is to estimate survival probability for each individual at a specific time-point. For a Weibull model this is simply provided by $1-F(x)$, with the CDF implemented already in base R as `pweibull`. 
...
There is also a function `test_survival_model` that checks the survival model contains the required attributes and methods, and that `predict_survival_probability` outputs sensible probabilities that are monotonically decreasing with time. If the test passes, it returns survival probabilties taken from random individuals in the supplied registry data at random time-points.
