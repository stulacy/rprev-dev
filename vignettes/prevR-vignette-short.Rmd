...
```{r}
status2 <- censor_at_index(registry_data$status, registry_data$eventdate, indexdate = "2013-01-30")
```
...
raw_incidence <- incidence(registry_data$entrydate, start="2004-01-01", 8)
raw_incidence
```
...
par(mfrow=c(2,2))
smoothed_incidence(registry_data$entrydate, start_date="2004-01-01", num_years=8)
```
...
registry_data_r <- registry_data[registry_data$entrydate >= "2004-01-01", ]
...
prevalence_total <- prevalence(Surv(time, status) ~ sex(sex) + age(age) + entry(entrydate),
                               registry_data, N_years = N_years, start='2005-09-01', num_years=8,
                               cure_time = cure*365)
