...
It can be concluded that the disease has around 100 new cases each year. To assess consistency of the incidence data with an homogeneous Poisson process, the `test_poisson_fit` and `smoothed_incidence` functions can be used:

```{r}
test_poisson_fit(raw_incidence)
```

**NOTE these are p-values for over and underdispersion**
...
To calculate the mean incidence rate per 100,000 within the study population `mean_incidence_rate` is used, which requires an estimate of the size of the population at risk:
...
functional_form_age(Surv(time, status) ~ age, prevsim_r)
```

### Weibull model

We model survival in our patient cohort using a Weibull distribution. This is calculated within the prevalence function, however, it is important to look at `summary(wb)` to assess the fit:
...
### Bootstrapped survival modelling

The `boot_eg` function can be used to provide a visual impression of the variance in survival models fitted to bootstrapped samples. Below, we look at the survival curves for a 65 year old male patient:

**Explain precisely what the colours and grey lines mean**

```{r bootstrapping_eg, fig.width = 7, fig.height = 4, error = TRUE}
...
**Hide hard to follow plotting code, just display the code which uses our library. Although debatable as they could be interested in seeing what is returned from function**
...
In the first instance, prevalence can be estimated by counting from the registry data. It is imperative that the number of incident cases is complete for estimating prevalence, and that follow-up data for all cases is complete at least until after the index date. In the case of the provided *prevsim* dataset where there is a discrepency between the last recorded incident case and the last date of follow-up owing to a systematic difference in methods of incidence and survival data collection, `r data.table::last(determine_registry_years(start='2004-01-30', num_reg_years=9))` is the index date. This is calculated to be the last date of the last year of registry data included by the user. In `prevalence_counted()`, all cases alive after the index date are censored. 
...
Prevalence is estimated using simulation by calling `prevalence()`. `num_years_to_estimate` corresponds to the number of years preceding the index date that contribute incident cases:

```{r prevalencetotal, error = TRUE}
prevalence_total <- prevalence(Surv(time, status) ~ sex(sex) + age(age) + entry(entrydate) + event(eventdate),
                               prevsim, num_years_to_estimate, population_size=1e6, cure, 
                               start='2004-01-30', num_reg_years=9)
