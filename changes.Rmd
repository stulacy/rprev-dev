...
inc_test_new <- function() {
  incidence(registry_data$DateOfDiag, sapply(5:13, function(x) sprintf("20%02d-09-01", x)))
}
```
...
Changes:
  ...
  + As with `incidence`, removed `registry_year` indices by instead requiring user pass in years of interest in `registry_year` vector. 
...
meanIR_test_dev <- function() {
  meanIR(registry_data$DateOfDiag, sapply(5:13, function(x) sprintf("20%02d-09-01", x)),
...
The test now fails as the returned objects are no longer comparable due to the dev function returning a list, but if we manually inspect the outputs we can see they are the same, aside from a precision difference in `per100K.lower`. This presumably arises from manually calculating the required z value for the confidence intervals, which is actually `r qnorm(0.975)`, rather than 1.96 as was previously hardcoded.
...
prev_dev <- function() {
  ...
  prevalence_total <- prevalence(load_data(registry_data), registry_years, 
                                 N_years = N_years, colnames=names,
                                 cure_time = cure*365)
}
```
...
countprev_dev <- function() {
  set.seed(17)
  registry_years = sapply(5:13, function(x) sprintf("20%02d-09-01", x))
  colnames = list(entry_date='DateOfDiag', event_date='EventDate', status_at_index='status2')
  counted_prevalence(registry_data, registry_years, colnames=colnames)
}
```
...
nyear_test_dev <- function() {
  ...
  n_year_estimates(N_years = 3, num_reg_years=length(registry_years)-1,
...
As expected the test fails as the outputs are lists.

```{r}
test_output(nyear_test_curr, nyear_test_dev)
```

However manual inspection demonstrates that they are calculating the same values (given the minor variation in the prevalence calculations).
...
prevage_test_dev <- function() {
    prev_obj <- prev_dev()
    post_age_dist_total <- prev_obj$post
    registry_end_year = 9
    N_years = 10
    prevalence_by_age(dist = post_age_dist_total, registry_end_year, N_years)
}
```
...
```{r}
prev_obj <- prev_dev()
prevalence_by_age(dist = prev_obj$post, registry_end_year=9, N_years=10, age_intervals=c(18, 50, 80))
```
