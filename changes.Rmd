...
meanIR_test_curr <- function() {
  ...
  meanIR_current(load_data(registry_data), registry_years, registry_start_year, 
                 registry_end_year, population_size=35e5)
}
```

```{r}
meanIR_test_dev <- function() {
  meanIR(registry_data$DateOfDiag, population_size=35e5, start='2005-09-01', num_years=8)
}
```
...
current_survival_rates <- function(x) {
  data(UKmortality)
  daily_survival_rate_males <- daily_survival_rate_current(UKmortality, sex = "Males")
...
runtime_comparison <- function() {
  ...
  data_r <- data[data$date_initial >= min(registry_years), ]
  form = Surv(survival_time, indicator) ~ age_initial + sex
  microbenchmark(registry_survival_bootstrapped(form, data, 1000, n_cores=1),
                 registry_survival_bootstrapped(form, data, 1000, n_cores=4),
...
As can be seen, the speed up resulting from these changes is substantial, although parallelising the execution doesn't offer any benefits for this dataset.
...
prev_current_output <- function() {
  ...
  daily_survival_rate_females <- daily_survival_rate_current(UKmortality, sex = "Females")
...
prev_dev <- function() {
  ...
  prevalence(Surv(stime, status) ~ sex(sex) + age(age) + entry(DateOfDiag),
             registry_data, N_years = N_years,
             start='2005-09-01', num_reg_years=8,
             cure_time = cure*365)
}
```
...
prev_male <- function() {
  ...
  N_years = 10
  prevalence(Surv(stime, status) ~ sex(sex) + age(age) + entry(DateOfDiag),
             subset(registry_data, sex==0), N_years = 10,
             start='2005-09-01', num_reg_years=8,
             cure_time = cure*365)
}
```
...
test_output(function() prev_current_output()[[1]], 
            function() prev_dev()$simulated_cases)
```
...
cases_curr <- data.frame(lapply(setNames(seq(dim(f_cur[[3]])[1]), seq(dim(f_cur[[3]])[1])), 
                         ...
                         method='current')
cases_dev <- data.frame(lapply(setNames(seq(dim(f_dev$samples)[1]), seq(dim(f_dev$samples)[1])), 
                               function(x) f_dev$samples[x, ]),
                        method='new')
...
post_cmp <- data.frame(age=c(as.vector(f_cur[[2]]), as.vector(f_dev$post_covar)),
                       method=rep(c('current', 'dev'), c(length(f_cur[[2]]), length(f_dev$post_covar))))
...
countprev_dev <- function() {
  counted_prevalence(registry_data$DateOfDiag, registry_data$status2, start='2005-09-01', num_reg_years=8)
}
```
...
Changes:
  ...
  + Made the function accept an object of class `prevalence` rather than three of the attributes which are already contained within it
...
nyear_test_dev <- function() {
  ...
  n_year_estimates(prevalence_d, N_years = 3,
                   population_size = 35e5)
}
```
...
test_nlphl <- function() {
    ...
    prevalence(Surv(stime, status) ~ sex(sex) + age(age) + entry(DateOfDiag),
               prelim_r, start='2007-09-01', num_reg_years=8,
               N_years = N_years,
               cure_time = cure*365)
}
```
