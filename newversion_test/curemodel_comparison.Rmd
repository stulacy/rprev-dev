...
**NB: one practical drawback of the mixture cure model is its extremely long fitting time, as I assume it is using EM to fit the latent $\pi(x)$ model. This means the overall prevalence estimation using 1000 simulations is an order of magnitude slower (speed comparison below) and so for a start could not be used for our website prevalence estimation.**
...
**Note that this has taken a significantly longer time to run than the default implementation, see the speed comparison below**
